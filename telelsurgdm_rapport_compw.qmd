---
title: "TELESUR-GDM"
subtitle: "Rapport Statistique -- Compléments V3"
author: 
...
cc(tt$icteron,tt$suivi, xlab = "Méthode de suivi", ylab = "Ictère", main = "Survenue d'un ictère selon la méthode de suivi", design = "cohort")
```
...
```{r}
#| label: tbl-ictere
#| tbl-cap: "Ictère : critères confondants"

glm(icteron ~ suivi + ddgage + imccalcul + diabgeston + macrosatcdon + parit + fingrospds, data = tt, family = "binomial") |>
    tbl_regression(exponentiate = TRUE) |>
    bold_labels() |>
    bold_p() |>
    pexptabph(nomsheet = "ictere", nomfich = classeur, exp = expx)
```
...
#| label: tbl-hypoict 
#| fig-cap: Hypoglycémie & Ictère

glm(suivi ~ hypoglicon* icteron, data = tt, family = "binomial") |>
    tbl_regression(exponentiate = TRUE) |>
    bold_labels() |>
    bold_p() |>
    pexptabph(nomsheet = "hypogly_ictere", nomfich = classeur, exp = expx)
```

Il n'y a pas de facteur de confusion entre hypoglycémie & ictère. 
